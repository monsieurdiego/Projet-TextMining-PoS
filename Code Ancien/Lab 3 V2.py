"""
Objectif : Quels mots sont les plus informatifs pour deviner leur classe ?
version beaucoup plus rapide en regroupant les mots par classe 
et en comptant leur fréquence globale, puis 
en appliquant une mesure simplifiée."""

"""à la base 
Je veux mesurer, pour chaque mot, à quel point sa présence est liée
 à une classe PoS. C’est là qu’on utilise une mesure appelée information mutuelle."""
 
"""ça prenait du temps car : 
Je fais ça pour chaque mot du corpus (≈ 50 000 mots),
 et à chaque fois je compares à tous les autres. 
Donc je fais 50 000 comparaisons de vecteurs de taille 50 000 → c’est très lourd.
Ainsi: Au lieu de calculer tous les scores en une seule fois 
(ce qui ne marche pas ici), je propose de calculer le score mot par mot,
 avec une boucle : 
...
C’est simple à comprendre :
 ...
 “ce mot est présent ici” → 1

“les autres mots ne sont pas lui” → 0

Je compares ça à la liste des classes

À la fin, on obtiens un score pour chaque mot.
Tu peux alors dire : “Je garde les mots avec un score ≥ 0.1” → ce sont les plus informatifs."""


"""Cette version 2 prend entre 15 et 20 min... Voir version 3""""
import os
import nltk
import pandas as pd
...
# Calcul de l'information mutuelle mot par mot
classes = df['pos'].tolist()
mi_scores = []
for i in range(len(df)):
    vecteur = [1 if j == i else 0 for j in range(len(df))]
    score = mutual_info_score(vecteur, classes)
    mi_scores.append(score)
